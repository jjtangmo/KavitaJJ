...
public class ProcessSeries : IProcessSeries {
    ...
    private async Task UpdateSeriesMetadata(Series series, Library library)
    {
        ...
        if(Parser.Parser.IsPdf(firstFile.FilePath)) {
            await MeModFunction.MangaSeriesGetMetaDataFromComicInfoXML_Pdf(series, _tagManagerService);
            return;
        }
    ...
    private async Task UpdateVolumes(Series series, IList<ParserInfo> parsedInfos, bool forceUpdate = false)
    {
        ...
        foreach(var volumeNumber in distinctVolumes) {
            ...
            foreach(var chapter in volume.Chapters) {
                ...
                if(firstFile == null || _cacheHelper.IsFileUnmodifiedSinceCreationOrLastScan(chapter, forceUpdate, firstFile))
                    continue;
                try {
    ...
    private async Task UpdateChapterFromComicInfo(Chapter chapter, ComicInfo? comicInfo, bool forceUpdate = false)
    {
        if(comicInfo == null)
            return;
        MangaFile? firstFile = chapter.Files.MinBy(x => x.Chapter);
        if(firstFile == null ||
            _cacheHelper.IsFileUnmodifiedSinceCreationOrLastScan(chapter, forceUpdate, firstFile))
            return;
