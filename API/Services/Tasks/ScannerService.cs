...
public class ScannerService : IScannerService {
    ...
    public async Task ScanSeries(int seriesId, bool bypassFolderOptimizationChecks = true)
    {
        ...
        if(series == null)
            return; // This can occur when UI deletes a series but doesn't update and user re-requests update
    ...
    public async Task ScanLibrary(int libraryId, bool forceUpdate = false, bool isSingleScan = true)
    {
        ...
        if(!await CheckMounts(library.Name, libraryFolderPaths))
            return;
        ...
        var (scanElapsedTime, processedSeries) = await ScanFiles(library, libraryFolderPaths, shouldUseLibraryScan, forceUpdate);
        ...
        bool isLibraryScan, bool forceChecks = false)
    {
        ...
        var processedSeries = await scanner.ScanLibrariesForSeries(library, dirs, isLibraryScan, await _unitOfWork.SeriesRepository.GetFolderPathMap(library.Id), forceChecks);
