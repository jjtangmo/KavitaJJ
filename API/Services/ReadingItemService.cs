...
public class ReadingItemService : IReadingItemService {
    ...
    private ParserInfo? Parse(string path, string rootPath, string libraryRoot, LibraryType type)
    {
        ...
        if(_bookParser.IsApplicable(path, type)) {
            var comicinfo = GetComicInfo(path);
            var parserInfo = _bookParser.Parse(path, rootPath, libraryRoot, type, comicinfo);
            //if(comicinfo is not null) {
            //    comicinfo.Title = MeModFunction.UseTitleByFileName(path);
            //    comicinfo.Series = MeModFunction.UseSeriesByFolder(parserInfo, rootPath, libraryRoot);
            //}
            return parserInfo;
        }
