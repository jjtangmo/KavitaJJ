...
public class ArchiveService : IArchiveService {
    ...
    {
        if(string.IsNullOrEmpty(archivePath) || !(File.Exists(archivePath) && Tasks.Scanner.Parser.Parser.IsArchive(archivePath) || Tasks.Scanner.Parser.Parser.IsEpub(archivePath)))
            return ArchiveLibrary.NotSupported;

        var ext = _directoryService.FileSystem.Path.GetExtension(archivePath).ToUpper();
        if(ext.Equals(".CBR") || ext.Equals(".RAR"))
    ...
    public static string? FirstFileEntry(IEnumerable<string> entryFullNames, string archiveName)
    {
        ...
        if(!string.IsNullOrEmpty(nonNestedFile))
            return nonNestedFile;
        ...
        if(!string.IsNullOrEmpty(firstDirectoryFile)) {
            ...
            if(!string.IsNullOrEmpty(firstDirectory)) {
                ...
                if(!string.IsNullOrEmpty(firstDirectoryResult))
                    return firstDirectoryResult;
            }
        }
    ...
    {
        if(string.IsNullOrEmpty(archivePath) || !IsValidArchive(archivePath))
            return string.Empty;
        try {
            var libraryHandler = CanOpen(archivePath);
            switch(libraryHandler) {
    ...
    public bool IsValidArchive(string archivePath)
    {
        ...
        if(Tasks.Scanner.Parser.Parser.IsArchive(archivePath))
            return true;
    ...
    /// <returns></returns>
    internal static ComicInfo? Deserialize(Stream stream)
    {
    ...
    private void ExtractArchiveEntries(ZipArchive archive, string extractPath)
    {
        ...
        if(!archive.HasFiles() && !needsFlattening)
            return;
    ...
    public void ExtractArchive(string archivePath, string extractPath)
    {
        ...
        if(_directoryService.FileSystem.Directory.Exists(extractPath))
            return;
